...
Remember: simplebuild_redirect.cfg
...
Bla::
  ...
  [project]
    ...
    # it a name (so they can specify it in their list of their depend.projects
    # list):
    #
    #  name = 'myprojectname'
    #
  ...
  [build]
    ...
    #   mode = 'debug'
    #
    # If you have many package, and would like to only consider a subset of them,
    ...
    #
    # Filters can use regular expressions, simple prefix them with 'RE::'. Here is
    ...
    # 'test'. The '(?i)' makes the latter case-insensitive, so 'MyUtilsTests' and
    # 'MyUtils_tests'would both be excluded. 'MyUtils' would be accepted, but
    # 'My_utils' would not:
    #
    #   pkg_filter = ['!RE::(?i).*test.*','RE::.*Util.*']
    #
    # Commas (m) can not be used in filters, but serve to separate filter
